...
module.exports = function (options) {
	...
	/**
	 * Checks if reqPath is a regular expression and stringifies it if it is.
	 * Returns a string with all instances of the pipe character replaced with
	 * the delimiter. Datadog does not support pipe characters in the metric string.
	 * @param  {*}       reqPath  The request path
	 *
	 * @return {string}           The request path with pipe chars replaced
	 */
	function replacePipeChar(reqPath) {
		if (reqPath instanceof RegExp) {
			reqPath = reqPath.toString();
		}

		return reqPath && reqPath.replace(REGEX_PIPE, DELIM);
	}
